...
public final class BasicFileUtils {
    ...
    private static final AtomicReference<File> NB_GRADLE_TEMP_DIR_REF = new AtomicReference<>(null);
    ...
    private static boolean isWriteable(File dir) {
        ...
        try {
            ...
            try (FileOutputStream output = new FileOutputStream(testTempFile)) {
                output.write(testByte);
                output.flush();
            }

            try (FileInputStream input = new FileInputStream(testTempFile)) {
